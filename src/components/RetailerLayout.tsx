...
function RetailerSidebar({ menuItems, profile }: { menuItems: any[], profile: any }) {
...
export default function RetailerLayout({ children, title, activePage, profile, headerActions }: RetailerLayoutProps) {
  const navigate = useNavigate();
  ...
  const menuItems = [
    {
      ...
      action: () => navigate("/retailer-dashboard"),
      active: activePage === "dashboard",
    },
    {
      title: "My Inventory",
      icon: Package,
      action: () => navigate("/retailer/inventory"),
      active: activePage === "inventory",
    },
    {
      title: "My Products",
      icon: PackageOpen,
      action: () => navigate("/retailer/products"),
      active: activePage === "products",
    },
    {
      title: "Purchase from Wholesalers",
      icon: Store,
      action: () => navigate("/retailer/wholesaler-marketplace"),
      active: activePage === "wholesaler-marketplace",
    },
    {
      title: "Wholesaler Orders",
      icon: ShoppingCart,
      action: () => navigate("/retailer/proxy-orders"),
      active: activePage === "wholesaler-orders",
    },
    {
  ...
  return (
    <SidebarProvider defaultOpen={true}>
      <div className="min-h-screen flex w-full bg-background">
        {/* Sidebar */}
        <RetailerSidebar menuItems={menuItems} profile={profile} />
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <header className="border-b bg-card h-16 flex items-center px-6 sticky top-0 z-40">
            ...
            <div className="flex items-center gap-2">
              {headerActions}
              <ThemeToggle />
              <Button variant="ghost" size="icon" onClick={() => navigate("/profile", { state: { from: 'retailer-dashboard' } })}>
